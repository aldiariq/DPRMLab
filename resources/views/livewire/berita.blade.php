<div>
    ...
    <div class="py-6">
        @if($modal)
        @include('livewire.modal.modalberita')
        @endif
                ...
                <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <span class="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
                            <button wire:click.prevent="delete({{$id_berita}})" type="button" class="inline-flex justify-center w-full rounded-md border border-transparent px-4 py-2 bg-red-600 text-base leading-6 font-medium text-white shadow-sm hover:bg-red-500 focus:outline-none focus:border-red-700 focus:shadow-outline-green transition ease-in-out duration-150 sm:text-sm sm:leading-5">
                                Hapus
                            </button>
                        </span>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr>
                                        <td class="px-6 py-4 text-sm text-gray-500 break-normal">
                                            <span class="font-medium">{{ $data->judul_beritas }}</span>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-500 break-normal">
                                            <span class="font-medium">{{ $data->tanggal_beritas }}</span>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-500 break-normal">
                                            <img class="h-20 w-20 rounded-full object-cover" src="{{ asset('storage/'.$data->foto_beritas)}}" />
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-500 break-normal">
                                            <span class="font-medium">{{ $data->isi_beritas }}</span>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-500 break-normal">
                                            <div class="flex item-center justify-center">
                                                <button wire:click="edit({{ $data->id_beritas }})" class="flex text-sm border-2 border-transparent transition">
                                                ...
                                                <button wire:click="showModaldelete({{ $data->id_beritas }})" class="flex text-sm border-2 border-transparent transition">
                                                    <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
